...
router.get('/', async function (req, res) {
    ...
    for (var i = 0; i < json.vacancies.length; i++) {
		if (json.vacancies[i].salaryTo && json.vacancies[i].salaryTo > 4150){
			...
			result += '<description><![CDATA[ <a href="https://www.cv.lv/vacancy/' + json.vacancies[i].id + '">Open</a><br/>'+json.vacancies[i].employerName+'<br/> ' + json.vacancies[i].positionContent + ' ]]></description>';
			result += '</item>';
		}
    }
    result += '</channel>';
